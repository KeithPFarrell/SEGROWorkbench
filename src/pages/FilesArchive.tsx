...
export const FilesArchive: React.FC = () => {
  ...
  const handleDownload = (file: typeof archiveFiles[0]) => {
    ...
    if (isPDF) {
      ...
      const metersAudited = Math.floor(Math.random() * 200) + 100;
      const qualityScore = Math.floor(Math.random() * 20) + 80;
      const issuesFound = Math.floor(Math.random() * 10);
      const status = ['Approved', 'Under Review', 'Completed'][Math.floor(Math.random() * 3)];
...
/F1 16 Tf
50 750 Td
(Data Quality Audit Report) Tj
0 -30 Td
/F1 12 Tf
(${file.market} - ${file.cycle}) Tj
0 -20 Td
(Generated: ${new Date().toLocaleDateString('en-GB')}) Tj
0 -40 Td
/F1 14 Tf
(AUDIT SUMMARY) Tj
0 -25 Td
/F1 11 Tf
(Meters Audited: ${metersAudited}) Tj
0 -18 Td
(Quality Score: ${qualityScore}%) Tj
0 -18 Td
(Issues Found: ${issuesFound}) Tj
0 -18 Td
(Status: ${status}) Tj
0 -35 Td
/F1 14 Tf
...
0 -25 Td
/F1 11 Tf
(1. Review data collection processes) Tj
0 -18 Td
(2. Implement additional validation checks) Tj
0 -18 Td
(3. Schedule follow-up audit in 3 months) Tj
0 -40 Td
/F1 9 Tf
(Report generated by SEGRO Sustainability Workbench) Tj
ET
endstream
endobj
6 0 obj
